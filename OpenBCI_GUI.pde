...
void processNewData() {
  ...
  double foo;
  boolean specfilt = true; // false = FFT spectrum of raw data, true = FFT spectrum of filtered data
  ...
  for (int Ichan=0;Ichan < nchan; Ichan++) {  
    ...
    float[] fooData_raw;
    fooData_raw = specfilt ? dataBuffY_filtY_uV[Ichan] : dataBuffY_uV[Ichan];
...
//    }
  ...
  if (!specfilt) {
    //apply additio nal processing for the time-domain montage plot (ie, filtering)
    eegProcessing.process(yLittleBuff_uV,dataBuffY_uV,dataBuffY_filtY_uV,fftBuff);
  }
