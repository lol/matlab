...
svmmodel = fitcecoc(concatData, concatLabel);
predict = resubPredict(svmmodel);
confMat = confusionmat(concatLabel, predict)
perc = bsxfun(@rdivide, confMat, sum(confMat, 2)) * 100

fprintf('\n--- KFold Crossvalidation ---\n');
confMatK = zeros(size(confMat));
CVMdl = fitcecoc(concatData, concatLabel, 'KFold', 10);
kpredict = kfoldPredict(CVMdl);
confMatK = confusionmat(concatLabel, kpredict)
