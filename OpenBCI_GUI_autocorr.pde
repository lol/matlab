...
void initSystem() {
  ...
  for (int Ichan=0; Ichan < nchan; Ichan++) { 
    ...
    //g
    //fftBuff[Ichan] = PSD ? new FFT(2*Nfft-1, openBCI.get_fs_Hz()) : new FFT(Nfft, openBCI.get_fs_Hz());
    if(PSD)
...
void initializeFFTObjects(FFT[] fftBuff, float[][] dataBuffY_uV, int N, float fs_Hz) {
  ...
  float[] fooData;
  float[] fooDataZeroPadded;
  float[] fooDataRev;
  float[] fooDataRevZeroPadded;
  float[] fooDataCorr;
  ...
  for (int Ichan=0; Ichan < nchan; Ichan++) {
    ...
    fooDataRev = dataBuffY_uV[Ichan];
    fooData = Arrays.copyOfRange(fooData, fooData.length-Nfft, fooData.length);
    fooDataRev = dataBuffY_uV[Ichan];
    fooDataRev = Arrays.copyOfRange(fooData, fooData.length-Nfft, fooData.length);
    //g
    if(PSD) {
      for(int i = 0, j = fooData.length - 1; i < fooData.length; i++, j--)
        fooDataRev[i] = fooData[j];
      
      fooDataRevZeroPadded = Arrays.copyOf(fooData, 2*fooData.length);
      fooDataZeroPadded = Arrays.copyOf(fooDataRev, 2*fooDataRev.length);
      Arrays.fill(fooDataZeroPadded, fooData.length, 2*fooData.length, 0);
      Arrays.fill(fooDataRevZeroPadded, fooDataRev.length, 2*fooDataRev.length, 0);
      
      fooDataCorr = fooDataZeroPadded;
      for(int i = 0; i < fooData.length; i++) {
        for(int j = 0; j <= i; j++) {
          fooDataCorr[i] += fooData[j] * fooDataRev[i - j];
        }
      }
      Arrays.fill(fooDataCorr, 2*fooData.length - 1, 2*fooData.length, 0); 
      fftBuff[Ichan].forward(fooDataCorr);
    }
    else
...
void processNewData() {
  ...
  for (int Ichan=0; Ichan < nchan; Ichan++) {  
    ...
    float[] fooData_raw = filtFFT ? dataBuffY_filtY_uV[Ichan] : dataBuffY_uV[Ichan];  //use the raw data or filtered data for FFT
    //g
